...
category: Dev Log
eyebrow: За кулисами
summary: Полное руководство по Ode — что это такое, как работает, как писать посты и публиковать их на Cloudflare Workers, Railway или GitHub Pages.
lang: ru
...
Cloudflare Workers раздаёт статические файлы через глобальную сеть CDN.

**Вариант А — Прямая загрузка:**

1. Откройте [dash.cloudflare.com](https://dash.cloudflare.com) → **Workers & Pages**
2. Новое приложение → **Pages** → **Direct Upload**
3. Перетащите папку `out/` в зону загрузки
4. Сайт будет доступен на `*.pages.dev` через несколько секунд

**Вариант Б — Интеграция с Git (автоматический деплой при push):**

Добавьте `wrangler.toml` в корень проекта:
...
Затем подключите репозиторий в панели Cloudflare:

1. **Workers & Pages** → **Создать** → **Подключить к Git**
...
Каждый `git push` в основную ветку запускает автоматическую сборку и деплой. Cloudflare выполняет `npm run build`, затем `npx wrangler deploy` читает `wrangler.toml` и публикует содержимое `out/` как статические ресурсы.

:::tip
Бесплатный тариф Cloudflare Workers включает 100 000 запросов в день и глобальную CDN-сеть. Для личного блога этого практически неограниченно.
:::

...
summary: A complete guide to Ode — what it is, how it works, how to write and manage posts, and how to deploy it to Cloudflare Workers, Railway, or GitHub Pages.
featured: true
lang: en
...
Cloudflare Workers serves static assets globally via its CDN. Two ways to deploy:

**Option A — Direct Upload (no Git required):**

1. Go to [dash.cloudflare.com](https://dash.cloudflare.com) → **Workers & Pages**
2. Create a new application → **Pages** → **Direct Upload**
...
First, add a `wrangler.toml` to the project root:
...
Then connect your repo in the Cloudflare dashboard:
...
Every `git push` to the main branch triggers an automatic build and deploy. Cloudflare runs `npm run build`, then `npx wrangler deploy` reads `wrangler.toml` and publishes the `out/` directory as static assets.

:::tip
Cloudflare Workers includes 100,000 requests/day on the free tier and global CDN distribution. For a personal blog this is effectively unlimited.
:::
